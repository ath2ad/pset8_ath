...
function chart()
{
    ...
    for (var i = 0; i < shuttle.seats.length; i++)
    {
        ...
        {
            html += "<li><b>" + shuttle.seats[i].name + ", " + shuttle.seats[i].house + "</b></li>";
        }
    }
    html += "</ol>";
    $("#chart").html(html);
...
{
    var helper = 0;
    // check to see if shuttle is within 30 m of any House
    for (var i in HOUSES)
    {
        var near = shuttle.distance(HOUSES[i].lat, HOUSES[i].lng);
        if (helper < 1)
        {
            // announce that you must be within 30 m of House for Dropoff
            $("#announcements").html("Dropoff requires the shuttle to be within 30 m of a House");
        }
        if (near < 30)
        {
            // check to see if any passenger gets out at house
            for (var j = 0; j < shuttle.seats.length; j++)
            {
                if (shuttle.seats[j] != null)
                {
                    if (HOUSES[shuttle.seats[j].house] == HOUSES[i])
                    {
                        // set appropriate seat to null
                        shuttle.seats[j] = null;
                       
                       // subtract a point and shout it from the roof tops
                        points--;
                        helper++;
                        $("#announcements").html("Successful Drop! Passengers left: " + points);
                        chart();
                    }
                }
            ...
            if (helper < 1)
            {
               $("#announcements").html("No one getting dropped at this house");
            }   
        }
    ...
    if (points <= 0)
    {
        $("#announcements").html("You have droped off all the passengers! You win!");
    }
}
...
function keystroke(event, state)
{
    ...
    // make an alert on movement
    if (state == true)
    {
        $("#announcements").html("VRRRRoooommmm");
    }
    // clear when movement stops
    else if (state == false)
    {
        $("#announcements").html(" ");
    }
    ...
    // speed up with the H or h button
    else if (event.keyCode == 104 || event.keyCode == 72)
    {
        shuttle.velocity += 2;   
        $("#announcements").html("increasing speed to: " + shuttle.velocity); 
        return false;
    }
    // speed down with the L or l button
    else if (event.keyCode == 108 || event.keyCode == 76)
    {
        shuttle.velocity -= 2;   
        $("#announcements").html("lowering speed to: " + shuttle.velocity);        
        return false;
    }
...
function pickup()
{
    ...
    for (var i in PASSENGERS)
    {
        ...
        if (near < 15)
        {
            ...
            for (var j = 0; j < shuttle.seats.length; j++)
            {
                // if the passenger is not a freshman
                if (PASSENGERS[i].house == null)
                {
                    // Alert that there is not a seat available 
                    $("#announcements").html("Sorry, no room for you frosh!");
                    break;
                }
                ...
                if (shuttle.seats[j] == null)
                {
                    ...
                    // add valid passenger to empty seat
                    shuttle.seats[j] = PASSENGERS[i];
                    // remove passenger from earth placemark
                    var features = earth.getFeatures();
                    features.removeChild(PASSENGERS[i].placemark);
                    // remove passenger from map mark
                    PASSENGERS[i].marker.setMap(null);  
                    
                    // Update to show new passenger  
                    chart();    
                    $("#announcements").html("Get on the Bus!");  
                    break;      
                }
                
                            
            }  
            if (PASSENGERS[i].house != null && shuttle.seats[j] == null)
            {
                // no open seats, let them know
                $("#announcements").html("Sorry we are full! Drop Someone off.");
                break;  
            }              
        }   
    }    
    // if nothing after iterating through all passengers, then Alert about distance 
    if (near > 15)
    { 
    $("#announcements").html("Must be within 15.0 meters of a passenger!");  
    }
}
...
/**
*Times and ends instance if you fail to drop-off passengers in designated time
*/
function timer()
{
        alert("You have 5 minutes to drop off every passenger or you are fired!");
        setTimeout(function()
                       {
                           if (points > 0)
                           {
                               alert("You are fired! Take a hike pal!");
                           }
                           else if (points < 1)
                           {
                               alert("Harvard appreciates your hard work, guy!");
                           }
                        },300000);          
}
